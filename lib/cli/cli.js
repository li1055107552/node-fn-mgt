...
function recursiveFileList(dir, ignore = []) {
    ...
    function walk(currentDir) {
        ...
        for (const file of files) {
            ...
            if (path.extname(filePath) != '.js' || ignore.some(entry => filePath.includes(entry.replace("/" || "\\", path.sep)))) {
...
function compile(cwd = process.cwd()) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        // 把这些方法取出来，fns[]
        if (matches !== null) {
            matches.forEach(e => fns.add(e.replace(/\s/g, "").replace("fnmgt", "").replace(/["'`\(\)\.]/g, "").replace(",", ".")))
        }

    }
    console.log(fns.keys())
    ...
    fns.forEach((v) => {
        ...
        else if(v.startsWith("__") && v.endsWith("__")){
